 ...
 * 
 * The timeline is developed in javascript as a Data Visualization Event Time Line.
 * 
...
(function(factory){
    ...
    var Timeline = function(container) {
        ...
        this.clientTimeOffset = 3600;   // difference between client time and the time
        ...
        this.step = new Timeline.StepDate(this.options);
     ...
     *                                 timeline. Optional.
     */
    Timeline.prototype.draw = function(data, options) {
     ...
     * @param {DataTable or JSON array} data
     */ 
    Timeline.prototype.setGroups = function (data) {
        this.deleteGroups();
     ...
     *                          {String} group
     */ 
    Timeline.prototype.updateData = function  (index, values) {
        var data = this.data;
     ...
     * @return {Number} index
     */ 
    Timeline.prototype.getItemIndex = function(element) {
        var e     = element,
            dom   = this.dom,
            items = this.items,
            index = undefined;
     ...
     *                           directly redrawn
     */
    Timeline.prototype.setVisibleChartRange = function(start, end, redraw) {
        if (start == undefined) {
    ...
    */ 
    Timeline.prototype.setVisibleChartRangeAuto = function() {
        var items    = this.items;
        ...
        if (startMin !== undefined && endMax !== undefined) {
            ...
            this.setVisibleChartRange(new XDate(startMin), new XDate(endMax));
        } else {
     ...
     * of the timeline
     */ 
    Timeline.prototype.setVisibleChartRangeNow = function() {
        var now = new Date();
     ...
     */
    Timeline.prototype.getVisibleChartRange = function() {
        var range = {
    ...
    */ 
    Timeline.prototype.redrawFrame = function() {
        var dom     = this.dom,
     ...
     */ 
    Timeline.prototype.redrawContent = function() {
        var dom = this.dom,
        size = this.size;
        ...
        if (!dom.content) {
            ...
            if (!params.onMouseDown) {
                ...
                Timeline.addEventListener(dom.content, "mousedown", params.onMouseDown);
            }    
            if (!params.onTouchStart) {
                params.onTouchStart = function (event) {me.onTouchStart(event);};
                Timeline.addEventListener(dom.content, "touchstart", params.onTouchStart);
            }
            if (!params.onMouseWheel) {
                params.onMouseWheel = function (event) {me.onMouseWheel(event);};
                Timeline.addEventListener(dom.content, "mousewheel", params.onMouseWheel);
            }
            if (!params.onDblClick) {
                params.onDblClick = function (event) {me.onDblClick(event);};
                Timeline.addEventListener(dom.content, "dblclick", params.onDblClick);
            }
        }
    ...
    */ 
    Timeline.prototype.redrawAxis = function() {
        var dom     = this.dom,
            options = this.options,
            size    = this.size,
            step    = this.step;
        ...
        var start = this.screenToTime(0),
            end   = this.screenToTime(size.contentWidth),
    ...
    */ 
    Timeline.prototype.redrawAxisCharacters = function () {
        ...
        if (!axis.characterMinor) {
            var text           = document.createTextNode("0"),
                characterMinor = document.createElement("DIV");
        ...
        if (!axis.characterMajor) {
            var text           = document.createTextNode("0"),
                characterMajor = document.createElement("DIV");
    ...
    */ 
    Timeline.prototype.redrawAxisStartOverwriting = function () {
    ...
    */ 
    Timeline.prototype.redrawAxisEndOverwriting = function () {
        var dom   = this.dom,
    ...
    */ 
    Timeline.prototype.redrawAxisHorizontal = function() {
     ...
     * @param {String} text
     */ 
    Timeline.prototype.redrawAxisMinorText = function (x, text) {
        var size       = this.size,
            dom        = this.dom,
    ...
    * @param {Number} x
    */ 
    Timeline.prototype.redrawAxisMinorLine = function (x) {
     ...
     * @param {String} text
     */ 
    Timeline.prototype.redrawAxisMajorText = function (x, text) {
        var size       = this.size,
            props      = size.axis.properties,
            frame      = this.dom.axis.frame,
     ...
     * @param {Number} x
     */ 
    Timeline.prototype.redrawAxisMajorLine = function (x) {
        var size       = this.size,
            props      = size.axis.properties,
            axis       = this.size.axis,
            frame      = this.dom.axis.frame,
     ...
     * Redraw all items
     */ 
    Timeline.prototype.redrawItems = function() {
        var dom          = this.dom,
            options      = this.options,
                ...
                switch (item.type) {
                    case 'range':
                        if (rangesUsed < rangesCreated) {
                            ...
                            // create a new range
                            var domItem = this.createEventRange(item.content, item.groupId);
        ...
        switch (item.type) {
            ...
            case 'box':
                ...
                var visible = ((left + item.width/2 > -contentWidth) && (left - item.width/2 < 2 * contentWidth));
                if (visible || size.dataChanged) {
                    // when data is changed, all items must be kept visible, as their heights must be measured
                    if (item.hidden) {
                        item.hidden = false; 
 ...
 *                            or HTML code.
 */ 
Timeline.prototype.createEventBox = function(content) {
  // background box
 ...
 *                            or HTML code.
 */ 
Timeline.prototype.createEventDot = function(content) {
  // background box
  var divBox = document.createElement("DIV");
  divBox.style.position = "absolute";
  ...
  return divBox;
     ...
     *                             or HTML code.
     */ 
    Timeline.prototype.createEventRange = function(content, groupId) {
        // background box
        var divBox            = document.createElement("DIV"),
            groupClass        = ( groupId ) ? " timeline-group-" + groupId : "";

        divBox.style.position = "absolute";
        divBox.className      = "timeline-event timeline-event-range" + groupClass;
 ...
 */ 
Timeline.prototype.redrawGroups = function() {
  var dom = this.dom,
    options = this.options,
    size = this.size,
    groups = this.groups;
 ...
 */ 
Timeline.prototype.redrawCurrentTime = function() {
  var options = this.options,
    dom = this.dom,
    size = this.size;
 ...
 */ 
Timeline.prototype.redrawCustomTime = function() {
  var options = this.options,
    dom = this.dom,
    size = this.size;
 ...
 */
Timeline.prototype.redrawDeleteButton = function () {
  var timeline = this,
    options = this.options,
    dom = this.dom,
    size = this.size,
 ...
 */
Timeline.prototype.redrawDragAreas = function () {
  var timeline = this,
    options = this.options,
    dom = this.dom,
    size = this.size,
 ...
 */
Timeline.prototype.redrawNavigation = function () {
  var timeline = this,
    options = this.options,
    dom = this.dom,
    frame = dom.frame,
  ...
  if (!navBar) {      
    if (options.editable || options.showNavigation) {
      ...
      navBar.className = "timeline-navigation btn-group";
    ...
    if (options.editable && options.showButtonAdd) {
      ...
      var onAdd = function(event) {
        Timeline.preventDefault(event);
        Timeline.stopPropagation(event);
      ...
      Timeline.addEventListener(navBar.addButton, "mousedown", onAdd); 
    ...
    if (options.showNavigation) {
      ...
      navBar.zoomInButton               = document.createElement("BUTTON");
      navBar.zoomInButtonIcon           = document.createElement("I");
      navBar.zoomInButton.className     = "timeline-navigation-zoom-in btn btn-inverse";
      navBar.zoomInButtonIcon.className = "icon-zoom-in";
      ...
      Timeline.addEventListener(navBar.zoomInButton, "mousedown", onZoomIn);
      navBar.zoomInButton.appendChild(navBar.zoomInButtonIcon);  
      navBar.appendChild(navBar.zoomInButton);
      
      // create a zoom out button
      navBar.zoomOutButton               = document.createElement("BUTTON");
      navBar.zoomOutButtonIcon           = document.createElement("I");
      navBar.zoomOutButton.className     = "timeline-navigation-zoom-out btn btn-inverse";
      navBar.zoomOutButtonIcon.className = "icon-zoom-out";
      ...
      Timeline.addEventListener(navBar.zoomOutButton, "mousedown", onZoomOut);
      navBar.zoomOutButton.appendChild(navBar.zoomOutButtonIcon);     
      navBar.appendChild(navBar.zoomOutButton);
        
      // create a move left button
      navBar.moveLeftButton               = document.createElement("BUTTON");
      navBar.moveLeftButtonIcon           = document.createElement("I");
      navBar.moveLeftButton.className     = "timeline-navigation-move-left btn btn-inverse";
      navBar.moveLeftButtonIcon.className = "icon-chevron-left";
      ...
      Timeline.addEventListener(navBar.moveLeftButton, "mousedown", onMoveLeft);
      navBar.moveLeftButton.appendChild(navBar.moveLeftButtonIcon);    
      navBar.appendChild(navBar.moveLeftButton);
      
      // create a move right button
      navBar.moveRightButton               = document.createElement("BUTTON");
      navBar.moveRightButtonIcon           = document.createElement("I");
      navBar.moveRightButton.className     = "timeline-navigation-move-right btn btn-inverse";
      navBar.moveRightButtonIcon.className = "icon-chevron-right";
      ...
      Timeline.addEventListener(navBar.moveRightButton, "mousedown", onMoveRight);
      navBar.moveRightButton.appendChild(navBar.moveRightButtonIcon);
 ...
 *                               example 1, 2, 5, or 10.
 */   
Timeline.prototype.setScale = function(scale, step) {
 ...
 */ 
Timeline.prototype.setAutoScale = function(enable) {
 ...
 */ 
Timeline.prototype.checkResize = function() {
 ...
 */ 
Timeline.filterImageUrls = function(elem, urls) {
  ...
  while (child) {
    ...
    Timeline.filterImageUrls(child, urls);
 ...
 */ 
Timeline.prototype.recalcSize = function() {
  var resized = false;
  ...
  if (size.dataChanged) {
    ...
    for (var i = 0, iMax = items.length; i < iMax; i++) {
      ...
      if (domItem) {
        Timeline.filterImageUrls(domItem, urls);
      }
    }
    if (urls.length) {
      for (var i = 0; i < urls.length; i++) {
        ...
        Timeline.imageloader.load(url, callback, sendCallbackWhenAlreadyLoaded);
      }
    }    
  }
 ...
 */ 
Timeline.prototype.recalcConversion = function() {
 ...
 *                      with the given date.
 */ 
Timeline.prototype.timeToScreen = function(time) {
  var conversion = this.conversion;
...
Timeline.prototype.onTouchStart = function(event) {
  ...
  if (!params.onTouchMove) {
    ...
    Timeline.addEventListener(document, "touchmove", params.onTouchMove);
  }
  if (!params.onTouchEnd) {
    params.onTouchEnd  = function (event) {me.onTouchEnd(event);};
    Timeline.addEventListener(document, "touchend",  params.onTouchEnd);
  }
};
...
Timeline.prototype.onTouchEnd = function(event) {
  ...
  if (params.onTouchMove) {
    Timeline.removeEventListener(document, "touchmove", params.onTouchMove);
    delete params.onTouchMove;
    
  }
  if (params.onTouchEnd) {
    Timeline.removeEventListener(document, "touchend",  params.onTouchEnd); 
...
Timeline.prototype.onMouseDown = function(event) {
  ...
  params.frameLeft = Timeline.getAbsoluteLeft(this.dom.content);
  params.frameTop = Timeline.getAbsoluteTop(this.dom.content); 
  ...
  params.target = Timeline.getTarget(event);
  ...
  if (!params.touchDown) {
    ...
    if (!params.onMouseUp) {
      ...
      Timeline.addEventListener(document, "mouseup", params.onMouseUp);
    }
 ...
 * This function activated from within the funcion Timeline.onMouseDown(). 
...
Timeline.prototype.onMouseUp = function (event) {
  ...
  if (params.onMouseUp) {
    Timeline.removeEventListener(document, "mouseup",   params.onMouseUp);
 ...
 * @param {event}  event
 */ 
Timeline.prototype.onDblClick = function (event) {
  var params = this.eventParams,
    options = this.options,
    dom = this.dom,
    size = this.size;
  ...
  if (params.itemIndex !== undefined) {
    ...
    // create a new item    
    var x = event.clientX - Timeline.getAbsoluteLeft(dom.content);
    var y = event.clientY - Timeline.getAbsoluteTop(dom.content);
        ...
        if (delta) {
            ...
            var zoom = function () {
                ...
                var zoomFactor     = delta / 5.0,
                    frameLeft      = Timeline.getAbsoluteLeft(timeline.dom.content),
                    zoomAroundDate = (event.clientX != undefined && frameLeft != undefined) 
 ...
 */ 
Timeline.prototype.zoom = function(zoomFactor, zoomAroundDate) {
 ...
 */ 
Timeline.prototype.move = function(moveFactor) {
  // zoom start Date and end Date relative to the zoomAroundDate
 ...
 * @param {Number} right
 */ 
Timeline.prototype.repositionItem = function (item, left, right) {
  var domItem = item.dom;
  switch(item.type) {
    case 'range':
      domItem.style.left = left + "px"; 
 ...
 */ 
Timeline.prototype.applyRange = function (start, end, zoomAroundDate) {
 ...
 */ 
Timeline.prototype.confirmDeleteItem = function(index) {
 ...
 * @param {int} index   Index of the item to be deleted
 */ 
Timeline.prototype.deleteItem = function(index) {
 ...
 * Delete all items
 */ 
Timeline.prototype.deleteAllItems = function() {
    this.unselectItem();
 ...
 */ 
Timeline.prototype.getGroupFromHeight = function(height) {
    var groups  = this.groups,
        options = this.options,
        size    = this.size,
 ...
 *                         {String} group
 */ 
Timeline.prototype.addItems = function (items) {
  var newItems = items,
    curItems = this.items,
    groups = this.groups,
  ...
  this.size.dataChanged = false;
     ...
     * @return {Object} item
     */ 
    Timeline.prototype.createItem = function(itemData) {
        var item = {
            //
            'id'        : itemData.id,
            'groupId'   : itemData.jobId,
 ...
 *                              {String} group (optional)
 */ 
Timeline.prototype.changeItem = function (index, itemData) {
  if (index >= this.items.length) {
 ...
 */ 
Timeline.prototype.findGroup = function (group) {
 ...
 * Delete all groups
 */ 
Timeline.prototype.deleteGroups = function () {
  this.groups = [];
  this.groupIndexes = {};
}
 ...
 */ 
Timeline.prototype.addGroup = function (groupName) {
  var groups = this.groups,
    groupIndexes = this.groupIndexes,
    groupObj = undefined;
 ...
 */ 
Timeline.prototype.cancelChange = function () {
 ...
 */ 
Timeline.prototype.cancelDelete = function () {
 ...
 */ 
Timeline.prototype.cancelAdd = function () {
  this.applyAdd = false;  
}
 ...
 */ 
Timeline.prototype.setSelection = function(selection) {
 ...
 * @param {Number} index
 */ 
Timeline.prototype.selectItem = function(index) {
  this.unselectItem();
 ...
 */ 
Timeline.prototype.isSelected = function (index) {
 ...
 */ 
Timeline.prototype.unselectItem = function() {
  if (this.selection) {
  ...
  this.selection = undefined;
     ...
     */ 
    Timeline.prototype.stackEvents = function(animate) {
        if (this.options.stackEvents == false || this.groups.length > 0) return;
        ...
        var sortedItems = this.stackOrder(this.items),
            finalItems  = this.stackCalculateFinal(sortedItems, animate);

        var sortedGroups      = this.groupOrder(this.items),
            finalGroupedItems = this.groupStackCalculateFinal(sortedGroups, animate);

        sortedItems = sortedGroups;
        finalItems  = finalGroupedItems;
     ...
     */ 
    Timeline.prototype.stackOrder = function(items) {
    ...
    /**
     * Order the items in the array this.items. The order is determined via:
     * - GroupId's, then ordered alpha
     * - The item with the left most location goes first
     * @param  {Array} items          Array with items
     * @return {Array} groupedItems   2D Array with sorted items in respective groups
     */ 
    Timeline.prototype.groupOrder = function(items) {
        var groups      = [],
            contents    = [],
            sortedItems = items.concat([]),
            groupedItems = [];

        for (var i = sortedItems.length - 1; i >= 0; i--) {
            if ( typeof groups[sortedItems[i].groupId] == 'undefined' ) {
                groups[sortedItems[i].groupId] = [];
                contents.push({
                    'groupId' : sortedItems[i].groupId,
                    'content' : sortedItems[i].content
                });
                
            }
            // push onto group array of item objects
            groups[sortedItems[i].groupId].push(sortedItems[i]);
        }
        ...
        for (var i = 0; i < contents.length; i++) {
            groupedItems.push(groups[contents[i].groupId]);
        }
     ...
     * other.
     */
    Timeline.prototype.stackCalculateFinal = function(items) {
        var size            = this.size,
            axisTop         = size.axis.top,
            options         = this.options,
            axisOnTop       = options.axisOnTop,
    ...
    /**
     * Adjust vertical positions of the events such that they don't overlap each
     * other in grouped mode.
     */
    Timeline.prototype.groupStackCalculateFinal = function(groups) {
        var size            = this.size,
            axisTop         = size.axis.top,
            options         = this.options,
            axisOnTop       = options.axisOnTop,
        ...
        for (var i = 0; i < groups.length; i++) {
            ...
            for (var j = 0; j < items.length; j++) {
                ...
                // calculate height based on order of groups
                if (axisOnTop) {
                    top = axisTop + eventMarginAxis + ( (height + ( eventMargin )) * (i + 1) );    
                } else {
                    top = axisTop - eventMarginAxis - ( (height + ( eventMargin )) * (i + 1) );    
                }
    ...
    *                               location, else false
    */ 
    Timeline.prototype.stackMoveOneStep = function(currentItems, finalItems) {
        var arrived = true;
        ...
        for (i = 0, iMax = finalItems.length; i < iMax; i++) {
            ...
            var topNow   = parseInt(item.top),
                topFinal = parseInt(finalItem.top),
                diff     = (topFinal - topNow);
 ...
 *                               positions of the items
 */ 
Timeline.prototype.stackMoveToFinal = function(currentItems, finalItems) {
 ...
 */ 
Timeline.prototype.stackEventsCheckOverlap = function(items, itemIndex, 
    itemStart, itemEnd) {
 ...
 */
Timeline.prototype.collision = function(item1, item2, margin) {
 ...
 */
Timeline.prototype.trigger = function (event) {
    // built up properties
    var properties = null;
    switch (event) {
    case 'rangechange':
    ...
    Timeline.events.trigger(this, event, properties);
}
...
Timeline.events = Timeline.events || {
  ...
  'trigger': function (object, event, properties) {
     ...
     */
    Timeline.StepDate = function(options) {
        // variables
        this.options = options;
        this.current = this.getDate();
        this._start  = this.getDate();
        this._end    = this.getDate();
        
        this.autoScale  = true;
        this.scale      = Timeline.StepDate.SCALE.DAY;
        this.step       = 1;

        // initialize the range
        this.setRange(options.start, options.end, options.minimumStep);
    }

    /// enum scale
    Timeline.StepDate.SCALE = { 
        MILLISECOND : 1, 
        SECOND      : 2, 
        MINUTE      : 3, 
        HOUR        : 4, 
 ...
 * 
 * Used in order to be able to centralize date functionality and
 * set things such as UTC vs automatic timezone calculation.
 */ 
Timeline.StepDate.prototype.getDate = function(date) {
    if ( this.options.utc ) {
        return ( date ) ? new XDate(date, true) : new XDate(true);
    } else {
        return ( date ) ? new XDate(date) : new XDate();
    }
}
     ...
     * @param {int}  minimumStep  Optional. Minimum step size in milliseconds
     */ 
    Timeline.StepDate.prototype.setRange = function(start, end, minimumStep) {
        if (isNaN(start) || isNaN(end)) return;

        this._start = ( start != undefined ) ? this.getDate(start) : this.getDate();
        this._end   = ( end != undefined ) ? this.getDate(end) : this.getDate();
 ...
 */ 
Timeline.StepDate.prototype.start = function() {
    this.current = this.getDate(this._start);
    this.roundToMinor();
}
 ...
 */ 
Timeline.StepDate.prototype.roundToMinor = function() {
    // round to floor
    // IMPORTANT: we have no breaks in this switch! (this is no bug)
    switch (this.scale) {
        case Timeline.StepDate.SCALE.YEAR:
            this.current.setFullYear(this.step * Math.floor(this.current.getFullYear() / this.step));
            this.current.setMonth(0);
        case Timeline.StepDate.SCALE.MONTH:        this.current.setDate(1);
        case Timeline.StepDate.SCALE.DAY:          this.current.setHours(0);
        case Timeline.StepDate.SCALE.HOUR:         this.current.setMinutes(0);
        case Timeline.StepDate.SCALE.MINUTE:       this.current.setSeconds(0);
        case Timeline.StepDate.SCALE.SECOND:       this.current.setMilliseconds(0);
        //case Timeline.StepDate.SCALE.MILLISECOND: // nothing to do for milliseconds
    }

    if (this.step != 1) {
        // round down to the first minor value that is a multiple of the current step size
        switch (this.scale) {
            case Timeline.StepDate.SCALE.MILLISECOND:  this.current.setMilliseconds(this.current.getMilliseconds() - this.current.getMilliseconds() % this.step);  break;
            case Timeline.StepDate.SCALE.SECOND:       this.current.setSeconds(this.current.getSeconds() - this.current.getSeconds() % this.step);  break;
            case Timeline.StepDate.SCALE.MINUTE:       this.current.setMinutes(this.current.getMinutes() - this.current.getMinutes() % this.step);  break;
            case Timeline.StepDate.SCALE.HOUR:         this.current.setHours(this.current.getHours() - this.current.getHours() % this.step);  break;
            case Timeline.StepDate.SCALE.DAY:          this.current.setDate((this.current.getDate()-1) - (this.current.getDate()-1) % this.step + 1);  break;
            case Timeline.StepDate.SCALE.MONTH:        this.current.setMonth(this.current.getMonth() - this.current.getMonth() % this.step);  break;
            case Timeline.StepDate.SCALE.YEAR:         this.current.setFullYear(this.current.getFullYear() - this.current.getFullYear() % this.step); break;
        }
    }
}
     ...
     */ 
    Timeline.StepDate.prototype.end = function () {
     ...
     * Do the next step
     */ 
    Timeline.StepDate.prototype.next = function() {
        ...
        if (this.current.getMonth() < 6)   {
            switch (this.scale) {
                case Timeline.StepDate.SCALE.MILLISECOND :
                    this.current = this.getDate(this.current.getTime() + this.step, true); 
                    break;
                case Timeline.StepDate.SCALE.SECOND :
                    this.current = this.getDate(this.current.getTime() + this.step * 1000);
                    break;
                case Timeline.StepDate.SCALE.MINUTE :
                    this.current = this.getDate(this.current.getTime() + this.step * 1000 * 60); 
                    break;
                case Timeline.StepDate.SCALE.HOUR :         
                    this.current = this.getDate(this.current.getTime() + this.step * 1000 * 60 * 60); 
                    // in case of skipping an hour for daylight savings, adjust the hour again (else you get: 0h 5h 9h ... instead of 0h 4h 8h ...)
                    var h = this.current.getHours();
                    this.current.setHours(h - (h % this.step));
                    break;
                case Timeline.StepDate.SCALE.DAY :          
                    this.current.setDate(this.current.getDate() + this.step); 
                    break;
                case Timeline.StepDate.SCALE.MONTH :        
            ...
            switch (this.scale) {
                case Timeline.StepDate.SCALE.MILLISECOND :
                    this.current = this.getDate(this.current.getTime() + this.step); 
                    break;
                case Timeline.StepDate.SCALE.SECOND :
        ...
        if (this.current.getTime() == prev) {
            this.current = this.getDate(this._end);
        }
    }
     ...
     */ 
    Timeline.StepDate.prototype.getCurrent = function() {
        return this.current;
    }
     ...
     *                               example 1, 2, 5, or 10.
     */   
    Timeline.StepDate.prototype.setScale = function(newScale, newStep) {
        this.scale = newScale;
     ...
     */ 
    Timeline.StepDate.prototype.setAutoScale = function (enable) {
     ...
     */ 
    Timeline.StepDate.prototype.setMinimumStep = function(minimumStep) {
        if (minimumStep == undefined) return;
        ...
        if (stepYear*1000 > minimumStep)        {this.scale = Timeline.StepDate.SCALE.YEAR;        this.step = 1000;}
        if (stepYear*500 > minimumStep)         {this.scale = Timeline.StepDate.SCALE.YEAR;        this.step = 500;}
        if (stepYear*100 > minimumStep)         {this.scale = Timeline.StepDate.SCALE.YEAR;        this.step = 100;}
        if (stepYear*50 > minimumStep)          {this.scale = Timeline.StepDate.SCALE.YEAR;        this.step = 50;}
        if (stepYear*10 > minimumStep)          {this.scale = Timeline.StepDate.SCALE.YEAR;        this.step = 10;}
        if (stepYear*5 > minimumStep)           {this.scale = Timeline.StepDate.SCALE.YEAR;        this.step = 5;}
        if (stepYear > minimumStep)             {this.scale = Timeline.StepDate.SCALE.YEAR;        this.step = 1;}
        if (stepMonth*3 > minimumStep)          {this.scale = Timeline.StepDate.SCALE.MONTH;       this.step = 3;}
        if (stepMonth > minimumStep)            {this.scale = Timeline.StepDate.SCALE.MONTH;       this.step = 1;}
        if (stepDay*5 > minimumStep)            {this.scale = Timeline.StepDate.SCALE.DAY;         this.step = 5;}
        if (stepDay*2 > minimumStep)            {this.scale = Timeline.StepDate.SCALE.DAY;         this.step = 2;}
        if (stepDay > minimumStep)              {this.scale = Timeline.StepDate.SCALE.DAY;         this.step = 1;}
        if (stepHour*4 > minimumStep)           {this.scale = Timeline.StepDate.SCALE.HOUR;        this.step = 4;}
        if (stepHour > minimumStep)             {this.scale = Timeline.StepDate.SCALE.HOUR;        this.step = 1;}
        if (stepMinute*15 > minimumStep)        {this.scale = Timeline.StepDate.SCALE.MINUTE;      this.step = 15;}
        if (stepMinute*10 > minimumStep)        {this.scale = Timeline.StepDate.SCALE.MINUTE;      this.step = 10;}
        if (stepMinute*5 > minimumStep)         {this.scale = Timeline.StepDate.SCALE.MINUTE;      this.step = 5;}
        if (stepMinute > minimumStep)           {this.scale = Timeline.StepDate.SCALE.MINUTE;      this.step = 1;}
        if (stepSecond*15 > minimumStep)        {this.scale = Timeline.StepDate.SCALE.SECOND;      this.step = 15;}
        if (stepSecond*10 > minimumStep)        {this.scale = Timeline.StepDate.SCALE.SECOND;      this.step = 10;}
        if (stepSecond*5 > minimumStep)         {this.scale = Timeline.StepDate.SCALE.SECOND;      this.step = 5;}
        if (stepSecond > minimumStep)           {this.scale = Timeline.StepDate.SCALE.SECOND;      this.step = 1;}
        if (stepMillisecond*200 > minimumStep)  {this.scale = Timeline.StepDate.SCALE.MILLISECOND; this.step = 200;}
        if (stepMillisecond*100 > minimumStep)  {this.scale = Timeline.StepDate.SCALE.MILLISECOND; this.step = 100;}
        if (stepMillisecond*50 > minimumStep)   {this.scale = Timeline.StepDate.SCALE.MILLISECOND; this.step = 50;}
        if (stepMillisecond*10 > minimumStep)   {this.scale = Timeline.StepDate.SCALE.MILLISECOND; this.step = 10;}
        if (stepMillisecond*5 > minimumStep)    {this.scale = Timeline.StepDate.SCALE.MILLISECOND; this.step = 5;}
        if (stepMillisecond > minimumStep)      {this.scale = Timeline.StepDate.SCALE.MILLISECOND; this.step = 1;}
    }
 ...
 */ 
Timeline.StepDate.prototype.snap = function(date) {
  if (this.scale == Timeline.StepDate.SCALE.YEAR) {
  ...
  else if (this.scale == Timeline.StepDate.SCALE.MONTH) {
    if (date.getDate() > 15) {
      date.setDate(1); 
  ...
  else if (this.scale == Timeline.StepDate.SCALE.DAY) {
    switch (this.step) {
      case 5:
      case 2: 
  ...
  else if (this.scale == Timeline.StepDate.SCALE.HOUR) {
    switch (this.step) {
      case 4:
    ...
    date.setSeconds(0);
    date.setMilliseconds(0);
  } else if (this.scale == Timeline.StepDate.SCALE.MINUTE) {
    switch (this.step) {
      case 15:
      case 10:
  ...
  else if (this.scale == Timeline.StepDate.SCALE.SECOND) {
    switch (this.step) {
      case 15:
      case 10:
  ...
  else if (this.scale == Timeline.StepDate.SCALE.MILLISECOND) {
 ...
 */ 
Timeline.StepDate.prototype.isMajor = function() {
  switch (this.scale)
  {
    case Timeline.StepDate.SCALE.MILLISECOND:
 ...
 */ 
Timeline.StepDate.prototype.getLabelMinor = function(date) {
  ...
  switch (this.scale)
  {
    case Timeline.StepDate.SCALE.MILLISECOND:  return String(date.getMilliseconds());
    case Timeline.StepDate.SCALE.SECOND:       return String(date.getSeconds());
    case Timeline.StepDate.SCALE.MINUTE:       return this.addZeros(date.getHours(), 2) + ":" +
                                                       this.addZeros(date.getMinutes(), 2);
    case Timeline.StepDate.SCALE.HOUR:         return this.addZeros(date.getHours(), 2) + ":" +
                                                       this.addZeros(date.getMinutes(), 2);
    case Timeline.StepDate.SCALE.DAY:          return String(date.getDate());
    case Timeline.StepDate.SCALE.MONTH:        return MONTHS_SHORT[date.getMonth()];   // month is zero based
    case Timeline.StepDate.SCALE.YEAR:         return String(date.getFullYear());
    default:                                         return "";    
  }
}
 ...
 */ 
Timeline.StepDate.prototype.getLabelMajor = function(date) {
 ...
 */ 
Timeline.StepDate.prototype.addZeros = function(value, len) {
  var str = "" + value;
 ...
 */ 
Timeline.addEventListener = function (element, action, listener, useCapture) {
 ...
 * @param {boolean}      useCapture
 */ 
Timeline.removeEventListener = function(element, action, listener, useCapture) {
 ...
 */ 
Timeline.getTarget = function (event) {
 ...
 */ 
Timeline.stopPropagation = function (event) {
  if (!event) 
    var event = window.event;
 ...
 */ 
Timeline.preventDefault = function (event) {
  if (!event) 
    var event = window.event;
 ...
 *                              in the browser page.
 */ 
Timeline.getAbsoluteLeft = function(elem) {
  var left = 0;
  while( elem != null ) {
 ...
 *                              in the browser page.
 */ 
Timeline.getAbsoluteTop = function(elem) {
  var top = 0;
  while( elem != null ) {
    top += elem.offsetTop;
    top -= elem.scrollTop;
