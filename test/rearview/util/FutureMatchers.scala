...
import rearview.util.RichThrowableImplicits._
import org.specs2.matcher._
import org.specs2.time.{ Duration => SpecsDuration }
import scala.concurrent.Await

trait AkkaConversions {
  implicit def specsDuration2Akka(duration: SpecsDuration): ScalaDuration = new DurationLong(duration.inMillis).millis
  implicit def specsDuration2Rich(duration: SpecsDuration) = new RichSpecsDuration(duration)
